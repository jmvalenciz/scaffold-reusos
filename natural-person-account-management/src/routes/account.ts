...
import { UpdateAccountBalanceValidator, type UpdateAccountBalance } from "../schemas/UpdateAccountBalance.js";
...
router.post("/update-customer-account-balance",
  describeRoute({
    description: "Retreive Natural Person data",
    tags: ["Natural Person"],
    responses: {
  ...
  validator('json', UpdateAccountBalanceValidator, (err, c)=>{
    if(!err.success){
      c.status(400);
      return c.json({error: "unable to parse body", message: JSON.parse(err.error.message)});
    }
  ...
  jwtValidatorMiddleware,
  async c=>{
    var {id, type, amount}: UpdateAccountBalance = await c.req.json();
    try {
      let person = await AccountController.updateAccountBalance(id, type, amount);
      c.status(200);
      return c.json({
        data: person,
        meta: {
